...
class MegaPi():
    ...
    def motorMove(self, leftSpeed, rightSpeed):
        self.__writePackage(
            bytearray([0xff, 0x55, 0x7, 0x0, 0x2, 0x5] + self.short2bytes(-leftSpeed) + self.short2bytes(rightSpeed)))
    ...
    def encoderMotorMove(self, slot, speed, distance, callback):
        ...
        self.__writePackage(bytearray(
            [0xff, 0x55, 0x0b, extId, 0x02, deviceId, 0x01, slot] + self.long2bytes(distance) + self.short2bytes(
                speed)))
    ...
    def encoderMotorMoveTo(self, slot, speed, distance, callback):
        ...
        self.__doCallback(extId, callback)
        self.__writePackage(bytearray(
            [0xff, 0x55, 0x0b, extId, 0x02, deviceId, 0x06, slot] + self.long2bytes(distance) + self.short2bytes(
                speed)))
    ...
    def stepperMotorMove(self, port, speed, distance, callback):
        ...
        self.__doCallback(extId, callback)
        self.__writePackage(bytearray(
            [0xff, 0x55, 0x0b, extId, 0x02, deviceId, 0x01, port] + self.long2bytes(distance) + self.short2bytes(
                speed)))
    ...
    def stepperMotorMoveTo(self, port, speed, distance, callback):
        ...
        self.__doCallback(extId, callback)
        self.__writePackage(bytearray(
            [0xff, 0x55, 0x0b, extId, 0x02, deviceId, 0x06, port] + self.long2bytes(distance) + self.short2bytes(
                speed)))
    ...
    def rgbledDisplay(self, port, slot, index, red, green, blue):
        self.__writePackage(
            bytearray([0xff, 0x55, 0x9, 0x0, 0x2, 18, port, slot, index, int(red), int(green), int(blue)]))
    ...
    def ledMatrixMessage(self, port, x, y, message):
        ...
        for i in range(len(arr)):
            arr[i] = ord(arr[i]);
        self.__writePackage(bytearray(
            [0xff, 0x55, 8 + len(arr), 0, 0x2, 41, port, 1, self.char2byte(x), self.char2byte(7 - y), len(arr)] + arr))
    ...
    def onParse(self, byte):
        ...
        if bufferLength >= 2:
            ...
            if (self.buffer[bufferLength - 1] == 0xa and self.buffer[
                bufferLength - 2] == 0xd and self.isParseStart == True):
                self.isParseStart = False
